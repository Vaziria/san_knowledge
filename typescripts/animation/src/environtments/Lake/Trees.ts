...
export class Trees extends THREE.Group {
  constructor(theme: Theme, keepClear: THREE.Vector3, wind: Wind) {
    super();
    this.name = 'trees';
    ...
    for (let attempt = 0; spots.length < COUNT && attempt < COUNT * 200; attempt++) {
      ...
      const Kind = deck[spots.length - 1];
      const tree = new Kind({ theme, seed: Math.floor(random() * 2 ** 31) });
